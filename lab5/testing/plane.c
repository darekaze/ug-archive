...
int main() {
    ...
    if (pid > 0) { //parent and control function
        ...
        continues running after the parent has terminated. Without this next for loop, it is possible to 
        get zombies. This is what waitpid is for. waitpid will force the current process to wait for another
        process to terminate before continuing. In the for loop, we force main to wait for each of its 
        children to terminate before continuing. */
    ...
    with the waitpid code, the last line shown will always be "There are no ZOMBIES".  If we remove the 
    waitpid section it will be possible to see output ater "There are no ZOMBIES", and it will even be 
    possible to see output after a new prompt line. */
...
void travel_agent(int *child_to_parent, int *parent_to_child, int id) {
    ...
    while (loop) {
        ...
        if (seats_left > 0) {
            printf("Child %d <%d> There are: %d seats, booking one!\n", id, getpid(), seats_left);
            seats_left--;
